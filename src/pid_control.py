...
""" We initialize first a class for the PID controller which has different publishers
these publishers publish to specific topics which the line follower uses to direct its path

"""
class PID(object):
    #first collecting initial setpoint value and initial state
    def __init__(self, init_setPoint_value, init_state):

        self.setPoint_value = init_setPoint_value
        self.state_value = init_state

        self._setpoint_pub = rospy.Publisher("/setpoint",Float64,queue_size=1) #setting up the publisher for the setpoint
        self._state_pub = rospy.Publisher("/state",Float64,queue_size=1)#setting up publisher for the state
        self._check_all_publishers_ready()#running publishers

        self._control_effort_sub = rospy.Subscriber('/control_effort', Float64, self.control_effort_callback)#the control effort 
